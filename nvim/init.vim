...
if !exists("$MYVIMFILES")
  if s:iswin
    let $MYVIMFILES = expand('~/AppData/Local/nvim')
  else
...
if !exists("$MYVIMRC")
  let $MYVIMRC = expand($MYVIMFILES . '/init.vim')
endif

if !exists("$MYGVIMRC")
  let $MYGVIMRC = expand($MYVIMFILES . '/gui_init.vim')
endif
...
" tmp directory.
if !exists('g:nvim_tmp_directory')
  let g:vim_tmp_directory = expand($MYVIMFILES . '/tmp')
endif
...
set backup
set nowritebackup
let &backupdir = expand(g:vim_tmp_directory . '/backup')

" Create swap files.
set swapfile
" Set swapfile save directory.
let &directory = expand(g:vim_tmp_directory . '/swap')

" Set undo file.
set undofile
let &undodir = expand(g:vim_tmp_directory . '/undo')
...
let s:local_config = expand($MYVIMFILES . '/local.vim')
if filereadable(s:local_config)
  source s:local_config
endif
